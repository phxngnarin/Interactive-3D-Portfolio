/**
 * main.js – สคริปต์หลักของฉาก 3D
 *   ไฟล์ที่ควรแก้เนื้อหา/รูป/สี  ->  config.js (อยู่โฟลเดอร์เดียวกับ index.html)
 */

// gsap และ howler โหลดเป็น <script> ธรรมดาไว้ใน index.html (ตัวแปร global)
...
/**
 * ROOM_SCALE — ย่อฉากทั้งหมดลงให้พื้นที่ห้อง (footprint) ไม่เกิน 10x10 หน่วย ตามเงื่อนไขอาจารย์
 * ขนาดเดิมของโมเดล (ไม่รวมพื้นหลัง/เงาที่ตั้งใจทำให้ใหญ่เกินห้อง) กว้างประมาณ 11.5 x 12.0 หน่วย
 * คูณ 0.82 แล้วจะเหลือประมาณ 9.4 x 9.9 หน่วย (ไม่เกิน 10x10 พร้อมเผื่อขอบเล็กน้อย)
 * ทุกพิกัดกล้อง/เป้าเล็งด้านล่างที่อ้างอิงตำแหน่งในโมเดล ถูกคูณด้วยค่านี้ให้สอดคล้องกันด้วย
 */
...
/**  -------------------------- Room material (day/night + tone remap) -------------------------- */
...
/**  -------------------------- Cel shading (Mr. Rabbit) -------------------------- */
// ห้องใช้แสง "อบ (baked)" ในเท็กซ์เจอร์อยู่แล้ว (ไม่มีไฟจริงในฉาก) เพิ่มไฟจริง 2 ดวงเบา ๆ
// ไว้ให้ MeshToonMaterial ของกระต่ายมีแสง-เงาแบบขั้นบันได (cel shading) ให้เห็นชัดเจน
// ไฟนี้แทบไม่กระทบวัสดุอื่น เพราะห้อง/แก้ว/หน้าจอ เป็น ShaderMaterial/MeshBasicMaterial ที่ไม่รับแสงอยู่แล้ว
...
const toonMaterial = new THREE.MeshToonMaterial({
  map: loadedTextures.day.Fourth, // สีเดิมของโมเดล (กระต่ายอยู่ในกลุ่มเท็กซ์เจอร์ Fourth)
...
/**  -------------------------- Computer screen (video + work carousel) -------------------------- */
...
// รูปนิ่ง (เช่น รูปตัวเอง) ที่โชว์บนจอตอน "ว่าง" (ไม่ได้เปิดดูผลงาน) — ตั้งค่าได้ที่ config.work.idleImage
// ถ้าไม่ตั้งไว้ (null) จะใช้วิดีโอ idleVideo แบบเดิม
...
// ตำแหน่งเดิมของไอคอน GitHub / YouTube / Twitter (ใช้คำนวณตำแหน่ง/มุมของป้ายข้อมูลติดต่ออันใหม่)
const contactSlots = [];
...
loader.load("assets/models/Room_Portfolio.glb", (glb) => {
  glb.scene.traverse((child) => {
    ...
    // ลบตุ๊กตา Kirby สีชมพูหลังเก้าอี้
    if (child.name.includes("Kirby")) {
      child.visible = false;
    }

    // ซ่อนตัวอักษรป้ายชื่อเดิม (จะสร้างใหม่ให้ตรงกับ config.signName)
    if (child.name.includes("Name_Letter")) {
      child.visible = false;
    }

    // ซ่อนไอคอน GitHub / YouTube / Twitter(X) เดิมทั้งหมด (จะแทนที่ด้วยป้ายข้อมูลติดต่อ 3D อันเดียว)
    ...
    if (child.name.includes("Water")) {
      ...
      // ตัวละครกระต่าย -> cel shading (MeshToonMaterial + gradient map แบบขั้นบันได)
      // ใช้ texture วันเดิมของฉากเป็นสี แต่แรเงาแบบ toon step ตาม toonLight ด้านล่าง
  ...
  // กรอบรูปทั้ง 3 ใบ ยังคงคลิกได้เหมือนเดิม (เนื้อหา 3D ข้างในจะถูกใส่หลังฟอนต์โหลดเสร็จ ด้านล่าง)
  ["Frame_1", "Frame_2", "Frame_3"].forEach((key) => {
    if (frameMeshes[key]) raycasterObjects.push(frameMeshes[key]);
  });

  // กรอบรูปถ่ายบนโต๊ะข้างคอมพิวเตอร์ (แทนที่การ์ดข้อมูลส่วนตัวเดิม) — ใส่รูปเองได้ที่ config.deskPhoto.image
  if (config.deskPhoto) {
    const deskFrame = createDeskPhotoFrame(config.deskPhoto.image, {
      width: 0.52,
      height: 0.66,
    });
    // ตำแหน่งเดิมของการ์ดข้อมูลส่วนตัว: บนโต๊ะ ด้านขวาของคอมพิวเตอร์ (ถัดจากแก้วกาแฟ)
    deskFrame.position.set(-2.32, 3.19 + 0.33, 0.25);
    deskFrame.rotation.y = Math.PI / 2; // หมุนให้หน้ากรอบหันออกมาทางกล้อง (แกน +x)
    deskFrame.rotation.x = -0.12; // เอียงหลังเล็กน้อยให้ดูเหมือนกรอบตั้ง
    roomGroup.add(deskFrame);
    registerBounce(deskFrame);
  }

  // ป้ายชื่อ + ป้ายข้อมูลติดต่อ 3D + ป้ายข้อมูล 3D บนกรอบ About (ทั้งหมดต้องรอฟอนต์โหลดเสร็จก่อน เพราะเป็นตัวอักษร 3D จริง)
  ...
  fontLoader.load("lib/fonts/helvetiker_bold.typeface.json", (font) => {
    ...
    // ป้ายไม้ข้อมูลติดต่อ 3D อันเดียว แทนที่ไอคอน GitHub / Facebook / Discord เดิมทั้งหมด
    if (contactSlots.length && config.contact?.sign) {
      ...
      // ยก/เลื่อนป้ายข้อมูลติดต่อได้ที่ config.contact.sign.offset (ไม่ต้องแก้ไฟล์นี้)
    ...
    // ป้ายไม้ข้อมูล 3D บนกรอบ About ทั้ง 3 ใบ (จาก config.about.frames)
...
function createDelayedHitboxes() {
  // ตัวเลือกนำร่องเผื่อโมเดลย่อยโหลดช้า — ปัจจุบันไม่มีอ็อบเจ็กต์เพิ่มเติมที่ต้องรอ
}
...
// ตำแหน่งกล้อง/เป้าเล็งดิบของมุม "My Work" (ก่อนย่อขนาดห้อง) — ใช้คำนวณระยะถอย/เข้าใกล้จอ
const WORK_CAMERA_BASE = { pos: [0.02, 5.24, -1.365], target: [-2.855, 4.492, -1.492] };

// คำนวณตำแหน่งกล้องมุม "My Work" ใหม่ทุกครั้งที่กด ตามค่า config.work.cameraPullBack
// (บวก = ถอยออกจากจอเพิ่ม, ลบ = เข้าใกล้จอมากขึ้น) โดยเล็งไปที่จอเดิมเป๊ะ ๆ ไม่เปลี่ยนมุมมอง
...
const CAMERA = {
  // work คำนวณสดทุกครั้ง เพื่อให้ config.work.cameraPullBack มีผลทันทีที่แก้ไฟล์ config.js
  get work() {
...
// centers ถูกคูณ ROOM_SCALE (เป็นตำแหน่ง), normal เป็นเวกเตอร์ทิศทาง (หน่วยยาว 1) จึงไม่คูณ
const FRAME_WORLD = {
...
function frameCameraSpot(key) {
  ...
  const dist = 1.55 * ROOM_SCALE; // ระยะห่างกล้อง-กรอบรูป ก็ต้องย่อตามฉากด้วย
  return {
...
const state = { mode: "default", index: null }; // mode: default | work | about | aboutDetail | contact
let cameraFree = true; // false while gsap is driving the camera outside OrbitControls
...
function handleClick() {
  ...
  // เดินขึ้นไปหา object ระดับบนสุดที่เรารู้จัก (เผื่อโดนลูกของกลุ่มโลโก้)
  let top = hit;
