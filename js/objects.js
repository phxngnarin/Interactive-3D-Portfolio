/**
 * objects.js – ของที่สร้างเพิ่มเองด้วยโค้ด (ไม่ได้อยู่ในไฟล์โมเดลเดิม)
 *   1. ตัวอักษร 3D บนป้ายชื่อไม้              -> createNameLetters()
 *   2. แผ่นไม้ + โลโก้ Facebook / Discord      -> createContactTile()  (ปัจจุบันไม่ได้ใช้แล้ว แต่คงไว้เผื่อใช้ในอนาคต)
 *   3. รูปที่วางทับในกรอบรูป (About)           -> addFramePicture()    (ปัจจุบันไม่ได้ใช้แล้ว แต่คงไว้เผื่อใช้ในอนาคต)
 *   4. ข้อความ 3D หลายบรรทัดบนแผ่นไม้           -> createPlateText()
 *   5. ป้ายไม้ข้อมูลติดต่อ 3D (แทนไอคอน Contact) -> createInfoSign()
 *   6. ป้ายไม้ข้อมูล 3D บนกรอบ About            -> createFrameInfoPlaque()
 *   7. กรอบรูปถ่ายบนโต๊ะ (ใส่รูปเองได้)          -> createDeskPhotoFrame()
 */
...
/* ------------------------------------------------------------------ */
/*  ทำให้ของที่สร้างเองมืดลงตอนเปลี่ยนเป็นโหมดกลางคืน                    */
/* ------------------------------------------------------------------ */
const tintables = [];
...
/* ------------------------------------------------------------------ */
/*  1) ตัวอักษรบนป้ายชื่อ                                                */
/* ------------------------------------------------------------------ */
// ตำแหน่งของป้ายไม้เดิม (แกนโลก) – ใช้วางตัวอักษรให้อยู่กลางป้าย
const SIGN = {
  centerX: 1.212,
  baselineY: 3.402,
  z: -4.215,
  maxWidth: 1.52, // ความกว้างสูงสุดที่ใส่ได้ (ระหว่างโลโก้ LoL กับไมค์)
  maxCapHeight: 0.17,
  depth: 0.05,
};

/**
 * @param {import("three/addons/loaders/FontLoader.js").Font} font  ฟอนต์ที่โหลดจาก FontLoader
 * @param {string} text      ข้อความบนป้าย
 * @returns {THREE.Mesh[]}   ตัวอักษรแต่ละตัว (ไม่รวมช่องว่าง)
 */
export function createNameLetters(font, text) {
  const chars = [...text.toUpperCase()];
  const tracking = 0.06; // ช่องไฟเพิ่ม (หน่วยเป็นสัดส่วนของขนาดฟอนต์)

  // วัดที่ size = 1 ก่อน แล้วค่อยย่อให้พอดีป้าย
  ...
  chars.forEach((ch, i) => {
    ...
    if (ch !== " ") {
      ...
      if (shapes.length) {
        ...
        // จัดให้ origin อยู่กึ่งกลางตัวอักษร (เหมือนตัวเดิมในโมเดล)
...
/* ------------------------------------------------------------------ */
/*  2) แผ่นไม้ + โลโก้ (Facebook / Discord)                              */
/* ------------------------------------------------------------------ */
// path ของโลโก้จาก simple-icons (สัญญาอนุญาต CC0) – viewBox 0 0 24 24
...
/** แปลง shape (แกน xy, ยื่นตามแกน z) ให้ไปอยู่บนหน้าแผ่นไม้ (หน้าแผ่น = +y, "บน" = -z) */
...
/**
 * สร้างแผ่นไม้พร้อมโลโก้ – ขนาด/ทิศทางเหมือนแผ่นเดิม (origin อยู่กึ่งกลางขอบล่าง)
 * @param {"facebook"|"discord"} kind
 * @param {THREE.Object3D} like  โหนดเดิมที่จะเอาตำแหน่ง/การเอียงมาใช้
 */
export function createContactTile(kind, like) {
  ...
  bodyGeo.translate(0, -depth - 0.006, 0); // หน้าแผ่นอยู่ที่ y = 0
  ...
  const build = (shps, d, bevel) => {
    ...
    g.translate(-12, -12, 0);
    g.scale(s, -s, s); // svg แกน y ชี้ลง -> พลิกให้ชี้ขึ้น
  ...
  // ชั้นหลังสีขาว (ให้เห็นเป็นตัว f / ตา ในโลโก้) – ย่อเล็กน้อยกันเส้นขอบซ้อนกัน
  let backShapes;
  if (kind === "facebook") {
    backShapes = [new THREE.Shape(new THREE.Path().absarc(12, 12, 12, 0, Math.PI * 2, false).getPoints(48))];
  } else {
    backShapes = shapes.map((sh) => new THREE.Shape(sh.getPoints(16))); // ไม่มีรู
  }
...
/*  3) รูปในกรอบรูป                                                      */
/* ------------------------------------------------------------------ */
// มุมของ "ภาพ" ในแต่ละกรอบ (พิกัด local ของโมเดลกรอบ) เรียงเป็น ซ้ายล่าง, ขวาล่าง, ขวาบน, ซ้ายบน
// (มองจากด้านหน้า) พร้อมเวกเตอร์ด้านหน้า – วัดจากเรขาคณิตในไฟล์ .glb
...
/** ครอปรูปให้เต็มกรอบ (เหมือน object-fit: cover) */
...
/**
 * วางรูปทับภาพเดิมในกรอบ  (ถ้าโหลดรูปไม่สำเร็จ จะเห็นภาพเดิมในโมเดลแทน)
 * @returns {{ mesh: THREE.Mesh, center: THREE.Vector3, normal: THREE.Vector3, width: number, height: number }}
 */
export function addFramePicture(frameMesh, key, url) {
  ...
  const lift = n.clone().multiplyScalar(0.004); // ยกขึ้นนิดหน่อยกันภาพซ้อนทับกัน (z-fighting)
  ...
  // ข้อมูลตำแหน่งภาพในพื้นที่โลก (ใช้ให้กล้องซูม)
...
/* ------------------------------------------------------------------ */
/*  4) ข้อความ 3D หลายบรรทัดบนแผ่นไม้ (ตัวอักษรสกัดเป็น 3D จริง ๆ         */
/*     ไม่ใช่ texture — รองรับ A-Z / a-z / 0-9 / สัญลักษณ์พื้นฐาน เท่านั้น)  */
/* ------------------------------------------------------------------ */

/** วัดความกว้างรวมของ 1 บรรทัด (หน่วยเดียวกับ size ที่ส่งเข้ามา) */
...
/** สร้าง mesh ตัวอักษร 3D ของ 1 บรรทัด วางเรียงจาก startX, baseline ที่ y */
...
/**
 * สร้างกลุ่มข้อความ 3D หลายบรรทัด (ตัวอักษรสกัดจริง ไม่ใช่รูปแบน) จัดกึ่งกลางทั้งแนวนอน/แนวตั้ง
 * ผลลัพธ์อยู่ใน "พิกัดท้องถิ่นของหน้าแผ่นไม้" (หน้าแผ่น = +y, บน = -z, ขวา = +x)
 * ให้เอาไปเป็นลูกของกลุ่มแผ่นไม้ที่วางตำแหน่ง/มุมไว้แล้วได้เลย
 * @param {import("three/addons/loaders/FontLoader.js").Font} font
 * @param {{text:string, size:number}[]} lines  แต่ละบรรทัด + ขนาดตัวอักษร (บรรทัดแรก = หัวข้อ มักใหญ่กว่า)
 * @param {{depth?:number, lineGap?:number, maxWidth?:number, color?:number, sideColor?:number, align?:"center"|"left"}} opts
 */
export function createPlateText(font, lines, opts = {}) {
  ...
  // ย่อขนาดทุกบรรทัดเท่า ๆ กัน ถ้าบรรทัดที่กว้างที่สุดล้นความกว้างที่กำหนด
  ...
  let y = totalH / 2 - sizedLines[0].size * 0.8; // baseline ของบรรทัดแรก (เผื่อ padding ด้านบน)
  ...
  // แปลงจากระนาบฟอนต์ปกติ (x=แนวนอน, y=แนวตั้ง) ให้ไปอยู่บน "หน้า" แผ่นไม้ (หน้า=+y, บน=-z)
  group.rotation.x = -Math.PI / 2;
  return group;
}

/** แผ่นไม้สี่เหลี่ยมมุมมน จัดกึ่งกลาง origin อยู่กึ่งกลางแผ่น (ต่างจาก TILE เดิมที่ origin อยู่ขอบล่าง) */
function centeredWoodPlate(width, height, opts = {}) {
  ...
  bodyGeo.translate(0, -height / 2, 0); // roundedRectShape สร้างจาก y:0..h -> เลื่อนให้กึ่งกลางอยู่ที่ origin
  bodyGeo.rotateX(-Math.PI / 2);
...
/* ------------------------------------------------------------------ */
/*  5) ป้ายไม้ข้อมูลติดต่อ 3D — แทนที่ไอคอน GitHub / Facebook / Discord   */
/* ------------------------------------------------------------------ */

/**
 * @param {import("three/addons/loaders/FontLoader.js").Font} font
 * @param {THREE.Object3D} like     โหนดเดิม (เช่น ไอคอนที่ถูกเอาออก) ใช้เอาตำแหน่ง/มุมมาวาง
...
/* ------------------------------------------------------------------ */
/*  6) ป้ายไม้ข้อมูล 3D บนกรอบ About (แทนที่รูปถ่ายเดิมในกรอบทั้ง 3)       */
/* ------------------------------------------------------------------ */

/**
 * วางป้ายไม้ + ข้อความ 3D ทับลงบนตำแหน่งกรอบรูปเดิม (ใช้พิกัด FRAME_PICTURES เดียวกับ addFramePicture)
 * @param {import("three/addons/loaders/FontLoader.js").Font} font
 * @param {THREE.Object3D} frameMesh  โหนดกรอบรูปเดิมจากโมเดล (Frame_1 / Frame_2 / Frame_3)
 * @param {"Frame_1"|"Frame_2"|"Frame_3"} key
 * @param {{title?:string, lines:string[]}} content
 */
export function createFrameInfoPlaque(font, frameMesh, key, content, opts = {}) {
  ...
  // แกน x=ขวา(right), y=ออกจากกรอบ(normal, ตรงกับ "หน้าแผ่น"), z=ลง(-up, ตรงกับ "บน = -z")
...
/* ------------------------------------------------------------------ */
/*  7) กรอบรูปถ่ายบนโต๊ะข้างคอมพิวเตอร์ (แทนที่การ์ดข้อมูลส่วนตัวเดิม)      */
/*     ใส่รูปเองได้ผ่าน config.deskPhoto.image                          */
/* ------------------------------------------------------------------ */

/** สี่เหลี่ยมมุมมนแบบมีรู (กรอบกลวงตรงกลาง) สร้างจาก y:0..h เหมือน roundedRectShape */
...
/**
 * สร้างกรอบรูปไม้ตั้งโต๊ะ (หน้ากรอบหัน +z, เหมือน PlaneGeometry ปกติ — ไม่ใช่พิกัดแบบแผ่นไม้ผนัง)
 * ผู้เรียกเป็นคนตั้ง position/rotation เอง (ดูตัวอย่างใน main.js ตรงที่เคยตั้งการ์ดข้อมูลส่วนตัว)
 * @param {string|null} url  path/URL รูป — ถ้าไม่มี จะโชว์กรอบเปล่า (สีกระดาษอ่อน ๆ) ไม่ทำให้เว็บพัง
 */
export function createDeskPhotoFrame(url, opts = {}) {
  ...
  // พื้นที่ใส่รูป (เต็มช่องว่างตรงกลางกรอบ) — สีกระดาษอ่อน ๆ ไว้ก่อนจนกว่ารูปจะโหลดเสร็จ
